		...
		xhr.onreadystatechange = function() {
			if(xhr.readyState == 4) {
				...
				for(var i = 0; i < vTxt.length; i++) {
					if(vTxt[i].number == 0) {
						temp += '<tr><td><input type="checkbox"/></td><td>' + vTxt[i].b_id + '</td><td>' + vTxt[i].b_name + '</td><td>' + vTxt[i].publish + '</td><td>' + vTxt[i].author + '</td><td>' + vTxt[i].press + '</td><td>' + vTxt[i].publish_at + '</td><td>' + vTxt[i].price + '</td><td>' + vTxt[i].number + '</td><td><a href="javascript:;" class="btn btn-default actionborrow" disabled>借书</a></td></tr>';
					} else {
						temp += '<tr><td><input type="checkbox"/></td><td>' + vTxt[i].b_id + '</td><td>' + vTxt[i].b_name + '</td><td>' + vTxt[i].publish + '</td><td>' + vTxt[i].author + '</td><td>' + vTxt[i].press + '</td><td>' + vTxt[i].publish_at + '</td><td>' + vTxt[i].price + '</td><td>' + vTxt[i].number + '</td><td><a href="javascript:;" class="btn btn-primary actionborrow">借书</a></td></tr>';
					}
				}
				$booktable.html(temp);
			}
	...
	$loginbtn.on('click', function() {
		...
		$.post("login.php", {
			...
			if(result == '1') {
				...
				$divlogin.html('<p class="regsuccess">login failed</p>');
				setTimeout(function() {
